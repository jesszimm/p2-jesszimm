...
public class MinFourHeap<E> extends PriorityWorkList<E> {
    ...
    public void add(E work) {
        ...
        if (size == capacity) { // if at capacity
            ...
            capacity +=  newCap;
            E[] temp = (E[])new Object[capacity]; //increase capacity by 1 level
        ...
        if (size != 0) {
            ...
            while (parent >= 0 && c.compare(data[child], (data[parent])) < 0) {
                E temp = data[parent];
    ...
    public E next() {
        ...
        while ((parent >= 0 && c.compare(data[parent], data[smallestChild]) > 0)) {
    ...
    private int findSmallestChild(int parent) {
        ...
        for (int i = 1; i < 5; i++) { //children = parent * 4 + (1, 2, 3, 4)
            if (4 * parent + i < size) {
                if (c.compare(data[4 * parent + i], (data[smallestChild])) < 0) {
